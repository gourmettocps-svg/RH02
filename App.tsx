...
function Toast({ notification, onClose }: { notification: Notification, onClose: () => void }) {
  ...
  // Script de reparo completo e idempotente
  const sqlFix = `-- 1. TABELAS\nCREATE TABLE IF NOT EXISTS public.users (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), name TEXT NOT NULL, email TEXT UNIQUE NOT NULL, password TEXT NOT NULL, role TEXT NOT NULL, created_at TIMESTAMPTZ DEFAULT now());\n\nCREATE TABLE IF NOT EXISTS public.employees (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), codigo TEXT, "nrRecibo" TEXT, name TEXT NOT NULL, "fatherName" TEXT, "motherName" TEXT, "birthDate" DATE, gender TEXT, "maritalStatus" TEXT, race TEXT, naturalness TEXT, nationality TEXT, address TEXT, neighborhood TEXT, "zipCode" TEXT, city TEXT, state TEXT, cpf TEXT UNIQUE NOT NULL, rg TEXT, "rgOrgao" TEXT, "rgEstado" TEXT, "rgEmissao" DATE, ctps TEXT, "ctpsSerie" TEXT, "ctpsEstado" TEXT, "ctpsExpedicao" DATE, pis TEXT, "pisDataCadastro" DATE, education TEXT, "cnhNum" TEXT, "cnhCat" TEXT, "cnhValidade" DATE, reservista TEXT, "reservistaCategoria" TEXT, "voterId" TEXT, "voterZone" TEXT, "voterSection" TEXT, sindicato TEXT, "consProfis" TEXT, "registroProfis" TEXT, "dataRegistroProfis" DATE, "bankInfo" JSONB DEFAULT '{}'::jsonb, "pixKey" TEXT, "admissionDate" DATE DEFAULT CURRENT_DATE, "fgtsOptant" BOOLEAN DEFAULT TRUE, "fgtsDataOpcao" DATE, "fgtsConta" TEXT, role TEXT, cbo TEXT, organograma TEXT, salary NUMERIC DEFAULT 0, "modoPgto" TEXT, "periodoPgto" TEXT, scale TEXT, "dismissalDate" DATE, relatives JSONB DEFAULT '[]'::jsonb, status TEXT DEFAULT 'Ativo', "performanceRating" INTEGER, notes TEXT, created_at TIMESTAMPTZ DEFAULT now());\n\nCREATE TABLE IF NOT EXISTS public.events (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), "employeeId" UUID REFERENCES public.employees(id) ON DELETE CASCADE, type TEXT NOT NULL, date DATE DEFAULT CURRENT_DATE, description TEXT, created_at TIMESTAMPTZ DEFAULT now());\n\n-- 2. RLS\nALTER TABLE public.users ENABLE ROW LEVEL SECURITY;\nALTER TABLE public.employees ENABLE ROW LEVEL SECURITY;\nALTER TABLE public.events ENABLE ROW LEVEL SECURITY;\n\n-- 3. POLÍTICAS (IDEMPOTENTES)\nDO $$\nBEGIN\n    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow public read on users') THEN CREATE POLICY "Allow public read on users" ON public.users FOR SELECT USING (true); END IF;\n    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow public read on employees') THEN CREATE POLICY "Allow public read on employees" ON public.employees FOR ALL USING (true); END IF;\n    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow public read on events') THEN CREATE POLICY "Allow public read on events" ON public.events FOR ALL USING (true); END IF;\nEND $$;\n\n-- 4. ADMIN\nINSERT INTO public.users (name, email, password, role) VALUES ('Admin', 'admin@gourmetto.com', '123', 'Gerente') ON CONFLICT (email) DO NOTHING;\n\n-- 5. RELOAD\nNOTIFY pgrst, 'reload schema';`;
             ...
             {isCacheError && (
               <div className="mt-4 p-3 bg-red-900/10 rounded-sm space-y-3">
                 ...
                 <button onClick={handleCopy} className="w-full bg-red-600 text-white py-2 rounded-sm font-bold text-[9px] uppercase flex items-center justify-center gap-2 hover:bg-red-700 transition-colors shadow-lg">
                   ...
                   {copied ? "Copiado!" : "Copiar Script Corrigido"}
                 </button>
               </div>
             )}
          </div>
